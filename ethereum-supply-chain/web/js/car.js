...
window.onload = async function () {
    ...
    for (var i = 0; i < products.length; i++) {
        var owners = await getOwnerHistoryFromEvents('TransferProductOwnership', products[i])
        console.log(owners)
        if (owners[owners.length - 1] == window.accounts[0]) {
            addItemToList(products[i], "car-list", carPartListManager)
        }
    }
            ...
            window.pm.methods.buildProduct(serial, "Car", creation_date, part_array).send({ from: window.accounts[0], gas: 2000000 }, function (error, result) {
                if (error) {
                    ...
                    for (var i = 0; i < part_list.length; i++) {
                        ...
                        // window.pm.methods.delPart(part_list[i])
                        // await window.pm.methods.delPart(part_list[i]).call({ from: window.accounts[0], gas: 2000000 })
                    }
                }
            })
        }
